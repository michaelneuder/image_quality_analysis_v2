...
def main():
    ...
    train_features, train_target, test_features, test_target = iqa_tools.load_data(local=True, path=data_path, dataset='cxs')
    print('images loaded ...')
    ...
    with tf.Session() as sess:
        ...
        for epoch_count in range(epochs):
            ...
            if epoch_count % 10 == 0:
                ...
                plt.savefig('cxs_error/cxs_error.png')
                np.savetxt('cxs_error/training_error_cxs.txt',training_error)
                np.savetxt('cxs_error/testing_error_cxs.txt', testing_error)

                shutil.rmtree('saved_models/cxs', ignore_errors=True)
                # Saving
                inputs = {
                ...
                tf.saved_model.simple_save(
                    sess, 'saved_models/cxs/'.format(epoch_count), inputs, outputs
                )
